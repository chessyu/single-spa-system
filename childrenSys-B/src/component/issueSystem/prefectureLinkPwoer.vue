<template>
    <Modal
        ...
        :closable="false"
        @on-ok="ok">
        <Form ref="prefectureLinkPwoer" :model="rowData" :label-width="80">
...
import { IP_GetZoneDept, IP_InsertDeptZone } from '@/api/issueSystem/prefectureLink.js'
...
export default{
    ...
    methods:{
        async ok(){
            ...
            let params = {
                zoneKey:this.rowData.zoneKey,
                deptIds:checkIds.map(e => e.deptId).join(','),
            };
            let data =  await IP_InsertDeptZone(params);
            if(data.code == 0){
        ...
        async getDeptList(){
            ...
            let selected = await IP_GetZoneDept({zoneKey: this.rowData.zoneKey});
            let array;
            if(selected.data){
