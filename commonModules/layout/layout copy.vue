    ...
    <Layout class="layout">
      <Sider ref="sideHeader" hide-trigger collapsible :collapsed-width="50" v-model="isCollapsed" class="layout__collapsible">
        <div class="layout-logo">
          <Icon :class="['layout__logo',isCollapsed?'layout__logo-big':'']" :type="systemTitle.systemIcon" />
          <span v-if="!isCollapsed">{{systemTitle.systemName}}</span>
        </div>
      ...
      <Layout class="ht__layout-content">
        <Header>
          ...
          <Breadcrumb v-if="getSysConfig.showBread" class="content_bread">
            <BreadcrumbItem v-for="(item,index) in bread" :key="index">{{item}}</BreadcrumbItem>
          </Breadcrumb>
          <div class="content-title">
            <div
              :class="['tit-item', 'plat_icon', i == 0 ? 'link__active' : '']"
              v-for="(item,i) in sonSystem"
              :key="i"
              :menuId="item.menuId"
              :sysCode="item.code"
              :sysIcon="item.icon"
              @click="selectSetem($event)"
            >
              <Icon :type="item.icon" />
              <span class="plat_span" :router="item.router">{{item.menuName}}</span>
            </div>
            <Dropdown class="tit-item user__item" @on-click="selectItem">
              <img src="../../assets/img/header.gif" />
              <span class="long_name">{{getUserData.userName}}</span>
        ...
        >© 2015-2019 All Rights Reserved. ****公司 http://www.hitalkers.com</Footer>
      </Layout>
    </Layout>
...
.layout-logo {
  ...
  .layout__logo{
    font-size: 25px;
    //background: linear-gradient(right, #fff700, red,#0037ff);
    // -webkit-background-clip: text;
    // color: transparent;
    // transition: all 0.2s ease 0.2s;
    // vertical-align: text-top;
    background-image: -webkit-linear-gradient(right, #2b58fb, #b7ce5aed 25%, #0037ff 50%, #b7ce5aed 75%, #2b58fb);
...
.link__active {
  color:  var(--activeElColor);
  // color: var(--themColor);
  // background: radial-gradient(var(--acitveBackColor), transparent);
  // background: linear-gradient(to right, #fff700, red,#0037ff);
  // -webkit-background-clip: text;
  // color: transparent;
